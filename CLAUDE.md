...
3) 배치: 항목당 남은 질문은 하나만(넣기의 OX가 첫 갈림길이므로).
   ...
   - 반복 O → "어떤 날마다?" : 아래 4가지 방식 제공.
     ...
     c. 기간으로 고르기: "3개월마다" 등 년/개월/일 간격 (근무 주기와 무관,
        ...
        만들 때는 오늘)
     d. 음력으로 고르기: "매년 음력 3월 15일마다" (부모님 생신 등 극소수
        용도. "기간" 화면과 똑같은 생김새로 숫자 두 칸(월/일)만 있음.
        새로 만들 때 원래 날짜의 음력을 자동으로 채워줌)
...
- 음력 매년 반복: "음력으로 고르기"(rule.type:'lunar', {lunarMonth, lunarDay}).
  부모님 생신처럼 아주 가끔 쓰는 용도라 가볍게 구현 — 새 화면 없이 "기간" 화면과
  같은 스피너 UI 재사용. 매년 그 음력 월/일의 "평달" 발생에만 맞춤(윤달 여부는
...
- events[]: 할 일 전체(한 번짜리 + 반복 할일 모두 여기 하나에 있음)
  ...
  - rule 종류: {type:'shift', shiftNames:[...], offset, weekdays?}
              ...
              {type:'lunar', lunarMonth, lunarDay}  (음력으로 고르기. 매년 그 음력
               월/일의 평달 발생에만 맞춤 — 윤달 무시)
  ...
  - 반복 규칙 고르는 화면(기간/근무로/요일로/음력으로)은 openRepeatEditor() 하나로 공용
