...
- events[]: 할 일 전체(한 번짜리 + 반복 할일 모두 여기 하나에 있음)
  ...
  - 반복 규칙 고르는 화면(기간[구석의 "음력" 토글로 양력/음력 전환]/근무·요일)은
    ...
    - **2026-07-16, 중요(★) 토글·원본 날짜/요일/근무 정보 줄을 한 번 없앴다가
      바로 되돌림**: 사용자 요청으로 지웠다가("이번 작업은 영 이상하네" —
      의도와 다르게 구현된 것으로 보임) 바로 복구함. promote-important-btn
      (입력칸 옆 ★/☆ 버튼)과 promote-source-date-info(제목 옆 "7월 16일
      (수) 주간" 같은 원본 정보 줄) 둘 다 원래대로 있음 — 다시 지우려면
      사용자가 정확히 뭘 원하는지 재확인 먼저 할 것(전에 한 번 오해해서
      되돌린 이력 있음).
...
## 화면 구성 (하단 탭 5개)
- 연간 / 달력(구 전체보기) / 오늘 / 반복 할일(구 루틴) / 설정 (+ 미배치 버튼은
