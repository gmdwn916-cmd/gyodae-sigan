...
- events[]: 할 일 전체(한 번짜리 + 반복 할일 모두 여기 하나에 있음)
  ...
  - 화면에 그릴 때는 반드시 buildTodoRow()/renderTodoList() 공용 함수로 그릴 것.
    ...
    완전히 같음 — 화면마다 다르게 만들지 말 것. **체크 원 크기(2026-07-18,
...
- **달력 드래그로 "할일 일괄 추가"(2026-07-19 추가)**: 근무 일괄 수정에
  ...
  - **"할일 추가" 시트는 근무 일괄 수정과 형식이 같음**(입력칸 + 색 스와치
    + 취소/적용). **색·모양은 근무 표시와 통일(2026-07-19, 같은 날 재조정)**
    — 처음엔 근무와 안 헷갈리게 `CATEGORY_COLOR_PRESETS`(카테고리 전용
    팔레트)를 쓰고 양끝이 둥근 알약 모양(`.cell-period-band`가 원래 갖고
    있던 모양)으로 일부러 다르게 만들었는데, 사용자가 "근무 색·톤을
    맞추고 근무처럼 좌우 꽉 채워 연결시켜달라"고 재요청 — 색 팔레트를
    `SHIFT_COLOR_PRESETS`로 바꾸고, `.cell-period-band` CSS 자체를
    `.cell-shift-band`와 같은 박스 모양(칸 좌우 여백만큼 늘려 옆 칸과 맞닿게,
    모서리 각지게, `width:calc(100% + 8px); margin:3px -4px 0;
    border-radius:0`)으로 바꿔서 근무 띠와 시각적으로 통일함(이 CSS 클래스는
    모든 기간(멀티데이) 할 일이 공유하므로, 예전에 `openPeriodEdit`로
    만들어진 것들도 자동으로 같은 모양이 됨 — 클래스가 하나뿐이라 별도
    분기 없이 전부 적용).
  ...
  - **기간 할 일에 `color` 필드 추가(2026-07-19, 하위 호환)**: 이 드래그로
    만든 할 일만 `ev.color`를 가짐 — `appendEventIndicators()`가 있으면
    그 색을, 없으면(예전 방식 등) 원래 고정 보라(`#5e5ce6`) 그대로 씀.
    기존 기간 할 일 편집 화면(`openPeriodEdit`, 텍스트/시작일/종료일만
    수정)은 색 수정 기능이 없음 — 색은 이 드래그 생성 시점에만 정해짐,
...
- 오늘 탭은 renderTodoList 대신 renderTodayTimeline으로 그림 — 미정(맨 위)·
  오전·오후·밤 4구역으로 나눠서 보여줌(위 timeSlot 설명 참고). **오후/밤
  ...
  시각을 다시 넣지 말 것). 이 구분은 오늘 탭에만 적용, 날짜 상세는 안 바뀜.
