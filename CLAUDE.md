...
- **달력 드래그로 "할일 일괄 추가"(2026-07-19 추가)**: 근무 일괄 수정에
  ...
  범위 자체를 지움(달력을 원래대로).
  - **"할일 추가" 시트는 근무 일괄 수정과 형식은 같지만(입력칸 + 색
    스와치 + 취소/적용), 근무와 헷갈리지 않게 의도적으로 구별함**: 색
    팔레트를 `SHIFT_COLOR_PRESETS`가 아니라 `CATEGORY_COLOR_PRESETS`
    (카테고리 기능이 삭제되면서 데이터 계층에만 남아있던 상수, 여기서
    재사용 — 근무색과 안 겹치는 팔레트라 이미 있던 걸 그대로 씀)로 다르게
    쓰고, 적용하면 근무처럼 옆 칸과 맞닿는 각진 띠(`.cell-shift-band`)가
    아니라 기존 "기간(멀티데이) 할 일"과 완전히 같은, 양끝이 둥근 알약
    모양 띠(`.cell-period-band`)로 그려져서 구조적으로도 근무와 구별됨.
