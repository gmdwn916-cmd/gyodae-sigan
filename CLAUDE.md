...
- **라이트/다크 배경-글자색 동기화**: 위젯 배경을 XML의 `@drawable` 참조로
  ...
  사고). 배경도 `context.getResources().getConfiguration().uiMode`로 매번
  직접 판단해서 `widget_background_light.xml`/`widget_background_dark.xml`
  (색이 고정된 채로 따로 있는 드로어블, `@color` 참조 없음) 중 하나를
  `setBackgroundResource`로 명시적으로 심을 것 — 다섯 위젯 전부 이 방식.
...
- **갱신 시점**: 근무·할일·설정에 영향 주는 모든 저장 시점(교대 주기, 주
  시작 요일, 근무유형 색, 날짜별/D번호 근무 수정, 카테고리, 할 일
  추가/수정/삭제 등)에서 pushAllWidgets()를 호출 — 위젯이 늘어나도 이 함수
...
- 근무·할일 계산은 위젯 2와 같은 함수 재사용(getEffectiveShiftName/
  ...
  pages: [{days(14)}]×3 } — days는 {date,dayNum,isToday,shiftName,color,todos}.
...
- **체크 반응 영역은 체크칸(item_check)만** — 텍스트(item_text)와 나머지
  ...
  항상 눌리도록 "보이지 않는 빈 줄"을 덧붙임(할 일이 정말 0개일 때는 안
  붙임 — today_empty 문구가 정확히 0개일 때만 뜨는 방식이라 건드리면 그
  문구가 안 뜨게 됨). **빈 줄 개수는 고정값이 아니라 위젯의 실제 크기를
  보고 계산함**(computeFillerCount(), 2026-07-16 — 처음엔 무조건 12개를
  붙였는데, 항목이 몇 개 없을 때 위젯 높이보다 빈 줄이 훨씬 많아져서 할
  일이 거의 없어도 목록이 "당겨서 스크롤"되는 상태가 돼버리는 문제가
  있었음, 사용자가 "실제로 할 일이 없다면 드래그가 안 됐으면 좋겠다"고
  요청). AppWidgetManager.getAppWidgetOptions()로 이 위젯의 실제 높이(dp)를
  읽어서 "지금 보이는 화면을 딱 채울 만큼만"(그 이상은 절대 안 채움 — 넘게
  채우면 다시 드래그가 가능해짐) 계산 — 항목이 이미 그 높이를 넘으면
  0(원래도 스크롤이 필요한 경우라 자연스러움), 모자라면 남는 만큼만. 줄
  높이·헤더 높이는 정확한 픽셀이 아니라 어림값(ROW_HEIGHT_DP=32,
  HEADER_HEIGHT_DP=34)이라 완벽히 딱 맞진 않을 수 있음 — 그래도 "터무니없이
  많은 빈 줄"은 확실히 막아줌. TodayWidgetService.onGetViewFactory()가
  인텐트에서 appWidgetId를 꺼내 팩토리에 넘겨줘야 이 계산이 가능함(예전엔
  안 넘겼었음). 위젯 크기를 리사이즈하면 TodayWidgetProvider.
  onAppWidgetOptionsChanged()가 목록을 새로고침해서 새 크기 기준으로 다시
  계산함.
