...
- 배포 = git add → commit → push. ("배포해줘"로 통칭 — 안드로이드 앱
  빌드와는 별개 개념, 아래 "작업 방식" 참고)
...
1. 코드 수정 전 항상 계획 먼저 제시, 사용자 확인 후에만 수정.
   ...
   - 계획을 승인받으면 그 뒤로는 세부 진행마다 다시 묻지 말고 끝까지 처리할 것.
...
7. 안드로이드 빌드는 코드 수정할 때마다 바로 하지 않고 모아둠 — 사용자가
   "빌드해줘"라고 명시적으로 말하기 전까지는 빌드 여부를 먼저 묻지도 말 것.
