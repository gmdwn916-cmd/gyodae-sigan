...
- **크기 4x1~4x5 리사이즈(2026-07-21 재설계)** — 처음엔 항상 정확히 2주
  ...
  - **1차 시도(실패, 같은 날 되돌림) — dp 값을 직접 읽어 계산**: 처음엔
    ...
    확신한 것이 오판이었음).
  - **2차 시도(현재, API 31+) — 반응형 위젯(SizeF 맵) API로 교체**: dp를
    ...
    런처와 안 맞는" 문제 자체가 발생할 수 없는 구조). 너비는 5개 항목 다
    똑같이 작은 값(`VARIANT_WIDTH_DP=100`, 이 위젯의 실제 minWidth 250dp보다
    항상 작아서 절대 비교에서 걸리지 않음 — 사실상 "너비는 신경 안 씀"
    효과)으로 고정하고, 높이만 "70×n-30" 값(40/110/180/250/320)으로
    구간을 나눔 — **이 절대값 자체가 실제 기기와 안 맞아도 상관없음**, 이
    5개는 서로 상대 비교(가장 큰 것 중 실제 크기 이하인 것을 고르는 방식)만
    되면 되기 때문(실제 1주 높이가 기기에서 90dp로 나오는 경우에도, "2주"가
    요구하는 110dp보다는 작으므로 "1주" 화면이 정확히 선택됨 — 절대값이
    다소 어긋나도 순서만 맞으면 안전한 구조). `updateOne()`은 이제
    `Build.VERSION.SDK_INT >= Build.VERSION_CODES.S`로 분기만 하고, 실제
    렌더링 로직은 `buildViewsForWeeks(context, appWidgetId, visibleWeeks)`
    (몇 주인지 파라미터로 받는 순수 렌더 함수)로 분리함. **API 31 미만(이
    프로젝트 minSdk 24)에서는 이 생성자 자체가 없어서 그런 기기는 항상
    `DEFAULT_VISIBLE_WEEKS=2`(2주) 고정** — 사용자가 "4x2는 2주치만 나오던
    전 버전으로 다시 돌리라"고 요청한 것과도 맞물려, 리사이즈를 안정적으로
    판단 못 하는(더 정확히는 API 자체가 없는) 기기에서는 아예 예전 고정
    동작으로 되돌아가는 게 더 안전하다고 판단.
  - **이전/다음 버튼이 몇 주씩 넘길지는 여전히 dp 어림 계산
    (`estimateVisibleWeeksForPaging()`)을 씀** — 렌더링과 달리 이건 "정확히
    맞을 필요"가 상대적으로 낮음(틀려도 그냥 조금 더/덜 넘어갈 뿐, 화면이
    깨지진 않음) — 렌더링(`buildViewsForWeeks`로 실제 몇 주를 보여줄지)과
    분리된 별개 계산이라는 점을 헷갈리지 말 것.
  - **앞으로 위젯 크기로 화면을 다르게 그려야 하는 기능을 또 만들 때**:
    "70×n-30" 같은 dp 공식으로 직접 역산하는 방식은 **위젯 선언 시 우리가
    고른 값과 실제 런처가 보고하는 값이 다를 수 있어 신뢰하지 말 것** —
    API 31+ 환경이 허용되면 이번처럼 `RemoteViews(Map<SizeF, RemoteViews>)`
